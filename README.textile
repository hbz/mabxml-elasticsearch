...
* @cd ../../../../ ; mvn exec:java -Dexec.mainClass="flow.Transform" -Dexec.args="src/main/resources/input/ bz2 lobid-hbz 193.30.112.170 hbz01"@

This will index the data from the specified directory to the cluster called 'lobid-hbz' via node '193.30.112.170' into an index called 'hbz01' (command above only works hbz internally).

You can then GET a specific record by hbz ID (the 'hbz01' part in those URLs are the index name from the transformation command above):
...
We run this transformation daily using a cron job. Internal documentation: To fully understand _what_ and _when_ it's done, trace the entries in crontab of _hduser@weywot1_.
